...
class FileUpload extends React.Component {
  constructor(props) {
    super(props);
    this.state = { fileUpload: null, isLoading: false };
    const MAX_FILE_SIZE_MB = 100;
    this.MAX_FILE_SIZE_KB = MAX_FILE_SIZE_MB * 1024 * 1024;
  }
  ...
  displayFileUpload = async (e) => {
    ...
    try {
      /* FIXME:
        The operating logic of this component allows for multiple file upload.
        It will correctly go through each selected file and display an error
      */
      if (file?.size >= this.MAX_FILE_SIZE_KB) {
        return;
      }
      ...
      const response = await fetch(`${this.props?.apiUrl}/files/upload`, {
        method: "POST",
        headers: {
  ...
  saveFile = async (e) => {
    ...
    const storedFileUrl = URL.createObjectURL(blob);
